...
from fame.serializers import (
    ...
    FameSerializer, BullshitterSerializer, ExpertSerializer,
)
...
class ExpertsApiView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        experts = api.experts()
        experts_serializer = ExpertSerializer(experts, many=True)
        return Response(experts_serializer.data, status=status.HTTP_200_OK)

    def post(self, request, *args, **kwargs):
        raise NotImplementedError()


class BullshittersApiView(APIView):
    permission_classes = [permissions.IsAuthenticated]

    def get(self, request, *args, **kwargs):
        bullshitters = api.bullshitters()
        print(bullshitters)
        bullshitters_serializer = BullshitterSerializer(bullshitters, many=True)
        return Response(bullshitters_serializer.data, status=status.HTTP_200_OK)
